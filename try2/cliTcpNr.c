...
void parseaza(int sd,int cod){
    if(cod == 1) inregistrare(sd);
    else if(cod == 2) login(sd);
    else if (cod == 3) sendIncident(sd);
    else if(cod == 4 ) setari(sd);
...
void userMenu(int sd){
  ...
  while(1){
    ...
    nr=atoi(buf);
    if(nr!=0) nr+=2;
    //TRIMITE OPTIUNE LA SERVER
    if(write(sd,&nr,sizeof(int))<=0){
        perror ("[server]Eroare la write() catre server.\n");
    }
    //PARSEAZA
    if(nr > 0) parseaza(sd,nr);
    else {
      printf("Ati iesit din cont. See ya!\n");
      break;
    }
...
void inregistrare(int sd){
    char username[30],parola[30],mesaj[100];
